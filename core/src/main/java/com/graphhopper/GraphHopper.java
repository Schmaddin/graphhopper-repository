...
import com.graphhopper.chilango.ChilangoWeightingLight;
...
public class GraphHopper implements GraphHopperAPI {
	...
	public GraphHopper() {
		...
		// order is important to use CH as base algo and set the approximation
		// in the followed lm factory decorator
	 ...
	 * path would go over the specified node count, important if none-CH routing
	 * is used.
	 */
	 ...
	 * <ul>
	 * <li>If no preferred language is specified, only the default language with
	 * no tag will be imported.</li>
	 * <li>If a language is specified, it will be imported if its tag is found,
	 * otherwise fall back to default language.</li>
	 * </ul>
	 */
	 ...
	 * OSMReader it is the OSM xml (.osm), a compressed xml (.osm.zip or
	...
	public GraphHopper init(CmdArgs args) {
		...
		if (args.has("osmreader.osm"))
			throw new IllegalArgumentException(
					"Instead osmreader.osm use datareader.file, for other changes see core/files/changelog.txt");
		...
		boolean eleCalcMean = args.has("graph.elevation.calcmean") ? args.getBool("graph.elevation.calcmean", false)
		...
		// osm import
		dataReaderWayPointMaxDistance = args.getDouble(Routing.INIT_WAY_POINT_MAX_DISTANCE,
				dataReaderWayPointMaxDistance);
	...
	protected DataReader createReader(GraphHopperStorage ghStorage) {
		throw new UnsupportedOperationException(
				"Cannot create DataReader. Solutions: avoid import via calling load directly, "
	...
	protected DataReader initDataReader(DataReader reader) {
		...
		return reader.setFile(new File(dataReaderFile)).setElevationProvider(eleProvider)
				.setWorkerThreads(dataReaderWorkerThreads).setEncodingManager(encodingManager)
	 ...
	 *            is the folder containing graphhopper files. Can be a
	 *            compressed file too ala folder-content.ghz.
	 */
	@Override
	public boolean load(String graphHopperFolder) {
		...
		if (!tmpFileOrFolder.isDirectory() && tmpFileOrFolder.exists()) {
			throw new IllegalArgumentException(
					"GraphHopperLocation cannot be an existing file. Has to be either non-existing or a folder.");
		} else {
			...
			if (compressed.exists() && !compressed.isDirectory()) {
				try {
					...
					throw new RuntimeException(
							"Couldn't extract file " + compressed.getAbsolutePath() + " to " + graphHopperFolder, ex);
				}
			}
		}
	...
	private void interpolateBridgesAndOrTunnels() {
		if (ghStorage.getEncodingManager().supports("generic")) {
			...
			logger.info("Bridge interpolation " + (int) sw.stop().getSeconds() + "s, " + "tunnel interpolation "
					+ (int) tunnel + "s");
		}
	}
	...
	public Weighting createWeighting(HintsMap hintsMap, FlagEncoder encoder, Graph graph) {
		...
		if (encoder.supports(GenericWeighting.class)) {
			...
			cMap = new GraphEdgeIdFinder(graph, locationIndex).parseStringHints(cMap, hintsMap,
					new DefaultEdgeFilter(encoder));
		...
		else if (ChilangoWeighting.name.equalsIgnoreCase(weighting)) {
			return new ChilangoWeighting(encoder, hintsMap);
		}else if (ChilangoWeightingLight.name.equalsIgnoreCase(weighting)) {
			return new ChilangoWeightingLight(encoder, hintsMap);
		}
	...
	/**
	 * This method calculates the alternative path list using the low level Path
	 * objects.
	 */
	public List<Path> calcPaths(GHRequest request, GHResponse ghRsp) {
		...
		try {
			...
			if (hints.has(Routing.EDGE_BASED))
				tMode = hints.getBool(Routing.EDGE_BASED, false) ? TraversalMode.EDGE_BASED_2DIR
			...
			if (algoStr.isEmpty())
				algoStr = chFactoryDecorator.isEnabled() && !forceFlexibleMode
						&& !(lmFactoryDecorator.isEnabled() && !hints.getBool(Landmark.DISABLE, false)) ? DIJKSTRA_BI
								: ASTAR_BI;

			List<GHPoint> points = request.getPoints();
			// TODO Maybe we should think about a isRequestValid method that
			// checks all that stuff that we could do to fail fast
			// For example see #734
			...
			for (int i = 0; i < maxRetries; i++) {
				...
				if (maxVisitedNodesForRequest > maxVisitedNodes)
					throw new IllegalArgumentException(
							"The max_visited_nodes parameter has to be below or equal to:" + maxVisitedNodes);
	...
	/**
	 * This method applies the changes to the graph specified as feature
	 * collection. It does so by locking the routing to avoid concurrent changes
	 * which could result in incorrect routing (like when done while a Dijkstra
	 * search) or also while just reading one edge row (inconsistent edge
	 * properties).
	 */
	...
	private void checkNonChMaxWaypointDistance(List<GHPoint> points) {
		...
		for (int i = 1; i < points.size(); i++) {
			...
			if (dist > nonChMaxWaypointDistance) {
				...
				detailMap.put("to", i);
				throw new PointDistanceExceededException(
						"Point " + i + " is too far from Point " + (i - 1) + ": " + point, detailMap);
			}
			lastPoint = point;
		}
	}
	...
	protected void prepareCH() {
		...
		if (tmpPrepare) {
			...
			if (chFactoryDecorator.getPreparationThreads() > 1 && dataAccessType.isMMap()
					&& !dataAccessType.isSynched())
				throw new IllegalStateException(
						"You cannot execute CH preparation in parallel for MMAP without synching! Specify MMAP_SYNC or use 1 thread only");
	...
	/**
	 * For landmarks it is required to always call this method: either it
	 * creates the landmark data or it loads it.
	 */
	...
	protected void cleanUp() {
		...
		PrepareRoutingSubnetworks preparation = new PrepareRoutingSubnetworks(ghStorage,
		...
		logger.info("edges: " + ghStorage.getAllEdges().getMaxId() + ", nodes " + currNodeCount + ", there were "
				+ preparation.getMaxSubnetworks() + " subnetworks. removed them => " + (prevNodeCount - currNodeCount)
				+ " less nodes");
	}
