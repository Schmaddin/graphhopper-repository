...
/**
 * This class parses an OSM xml or pbf file and creates a graph from it. It does
 * so in a two phase parsing processes in order to reduce memory usage compared
 * to a single parsing processing.
 * <p>
 ...
 * intermediate data structure for the pillar nodes (pillarLats/pillarLons) or,
 ...
 * intermediate data structure will be stored in the way geometry of that edge.
 * <p>
 *
 * @author Peter Karich
 */
public class OSMReader implements DataReader {
	...
	// TODO eigenes
	// private GHLongHashSet osmNodeIdSet = new GHLongHashSet();
	protected IntLongMap nodeIdToOsmNodeIdMap;
	protected List<String> info=new LinkedList<>();
	// TODO
	...
	public void readGraph() throws IOException {
		...
		LOGGER.info("time pass1:" + (int) sw1.getSeconds() + "s, " + "pass2:" + (int) sw2.getSeconds() + "s, "
	...
	// TODO eigenes
	/*
	 * private LongSet getOsmNodeIdSet() { return osmNodeIdSet; }
	 */
	...
	private void writeOsm2Graph(File osmFile) {
		...
		try {
			...
			if (in.getUnprocessedElements() > 0)
				throw new IllegalStateException(
						"Still unprocessed elements in reader queue " + in.getUnprocessedElements());

			// logger.info("storage nodes:" + storage.nodes() + " vs. graph
			// nodes:" + storage.getGraph().nodes());
		} catch (Exception ex) {
		...
		finishedReading();
		if (graph.getNodes() == 0)
			throw new RuntimeException("Graph after reading OSM must not be empty. Read " + counter + " items and "
					+ locations + " locations");
	}
	...
	void processWay(ReaderWay way) {
		...
		// looks with the encoding Manager, whether way is accepted (then returns acceptBit)
		...
		// Estimate length of ways containing a route tag e.g. for ferry speed
		// calculation
		...
		if (way.getTag("duration") != null) {
			try {
				...
				// Provide the duration value in seconds in an artificial
				// graphhopper specific tag:
	...
	/**
	 * @return OSM way ID from specified edgeId. Only previously stored
	 *         OSM-way-IDs are returned in order to reduce memory overhead.
	 */
...
//		info.add(""+osmId+"   lat: "+lat+" lon: "+lon+"       nextTowerId: "+nextTowerId);
//		info.add("  ");
	 ...
	 * in the graph.
	 */
	Collection<EdgeIteratorState> addOSMWay(final LongIndexedContainer osmNodeIds, final long flags,
			final long wayOsmId) {
	...
	protected EdgeIteratorState addEdge(int fromIndex, int toIndex, PointList pointList, long flags, long wayOsmId) {
		// sanity checks
		if (fromIndex < 0 || toIndex < 0)
			throw new AssertionError("to or from index is invalid for this edge " + fromIndex + "->" + toIndex
					+ ", points:" + pointList);
		if (pointList.getDimension() != nodeAccess.getDimension())
			throw new AssertionError("Dimension does not match for pointList vs. nodeAccess " + pointList.getDimension()
		...
		if (Double.isNaN(towerNodeDistance)) {
			LOGGER.warn("Bug in OSM or GraphHopper. Illegal tower node distance " + towerNodeDistance
					+ " reset to 1m, osm way " + wayOsmId);
			towerNodeDistance = 1;
		}
		...
		if (Double.isInfinite(towerNodeDistance) || towerNodeDistance > maxDistance) {
			...
			// so we can avoid the complexity of splitting the way for now (new
			// towernodes would be required, splitting up geometry etc)
	...
	protected void storeOsmNodeID(int nodeId, long osmNodeId) {
		// if (osmNodeIdToInternalNodeMap.get(osmNodeId)!=0) {
	...
	private void printInfo(String str) {
		LOGGER.info("finished " + str + " processing." + " nodes: " + graph.getNodes() + ", osmIdMap.size:"
				+ getNodeMap().getSize() + ", osmIdMap:" + getNodeMap().getMemoryUsage() + "MB" + ", nodeFlagsMap.size:"
				+ getNodeFlagsMap().size() + ", relFlagsMap.size:" + getRelFlagsMap().size() + ", zeroCounter:"
				+ zeroCounter + " " + Helper.getMemInfo());
	}
